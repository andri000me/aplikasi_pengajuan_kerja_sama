...
class M_kerja_sama_eksternal extends CI_Model
{
    ...
    function get_kerja_sama_eksternal_by_kategori($id_kategori_kerja_sama){
        $hasil=$this->db->query("SELECT * FROM kerja_sama_eksternal 
        JOIN user ON kerja_sama_eksternal.id_lembaga_mitra = user.id
        JOIN kategori_kerja_sama ON kerja_sama_eksternal.id_kategori_kerja_sama = kategori_kerja_sama.id_kategori_kerja_sama
        JOIN status_kerja_sama ON kerja_sama_eksternal.id_status_kerja_sama = status_kerja_sama.id_status_kerja_sama WHERE kerja_sama_eksternal.id_kategori_kerja_sama='$id_kategori_kerja_sama'");
        return $hasil;
    }
    function get_kerja_sama_eksternal_pengusul_by_kategori($id_kategori_kerja_sama){
        $hasil=$this->db->query("SELECT nama_mitra as nama_pengusul FROM kerja_sama_eksternal 
        JOIN user ON kerja_sama_eksternal.id_pengusul = user.id WHERE kerja_sama_eksternal.id_kategori_kerja_sama='$id_kategori_kerja_sama'");
        return $hasil->result_array();
    }
